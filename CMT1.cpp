    ...
    : QThread(parent)
{
    ...
    top = cv::Point2f(100,100);
    bottom = cv::Point2f(400,400);
    starting = true;
}
...
void CMT1::run()
{
        processFrame(im_gray);
}

void CMT1::initialise(cv::Mat im_gray0, cv::Point2f topleft, cv::Point2f bottomright)
{
...
void CMT1::estimate(const std::vector<std::pair<cv::KeyPoint, int> >& keypointsIN, cv::Point2f& center, float& scaleEstimate, float& medRot, std::vector<std::pair<cv::KeyPoint, int> >& keypoints)
{
...
void CMT1::processFrame(cv::Mat im_gray)
{
